	...
	while (1) {
		...
		if (flag == 0) {
			...
			if (ethhead != NULL) {
				...
				//IPADD <----> TierAddr Dissemination
				if (checkMSGType == 5) {
					
					/*Functionality
					1.The Edge node changes will be broadcaset to all the neighbor nodes
					2.The msgType will be 5
					3.
					4.
					5.
					*/
					...
					//ethhead[15] is number of entries	
					uint8_t totalEntries = ethhead[15];

					// Action to be performed
					//ethhead[16] is the add or delete
					uint8_t action = ethhead[16];

					int index = 17;
					int hasDeletions = 0;
					//if total entries > 0 ;then add in to the ipadd<---> Tier table
